...
<br/> 4. Move the player in that direction (or the opposite if the accelaration is negative)
...
<br/> We used the real speed check and also searched the scene for duplicate locomotion scripts.
<br/> Once only one script was moving the rig, the limit behaved normally again.
<br/>
<br/> 4 Problem: The text appearnce was not exacltly as we planned: wrong TextMesh material, mirrored text, font size, etc..
<br/> 4 Solution: We stopped doing “LookAt every frame” and used a local rotation, chaned material and font.
<br/>
<br/> 5 Problem: teleport arc/pointer from Oculus locomotion ( pink)
